...
from ..models import DatabaseStopDatabaseVM
from notification.tasks import TaskRegister


class DatabaseStopDatabaseVMAdmin(DatabaseMaintenanceTaskAdmin):
    search_fields = (
    ...
    def maintenance_action(self, maintenance):
        ...
        url = "/admin/maintenance/databasestopdatabasevm/{}/retry/".format(
            maintenance.id
        )
    ...
    def get_urls(self):
        base = super(DatabaseStopDatabaseVMAdmin, self).get_urls()

        admin = patterns(
            '',
            url(
                r'^/?(?P<stop_database_vm_id>\d+)/retry/$',
    ...
    def retry_view(self, request, stop_database_vm_id):
        retry_from = get_object_or_404(DatabaseStopDatabaseVM, pk=stop_database_vm_id)
        ...
        if not retry_from.is_status_error:
            ...
            messages.add_message(
                ...
                "You can not do retry because stop database vm status is '{}'".format(
        ...
        if not retry_from.can_do_retry:
            error = True
            messages.add_message(
                request, messages.ERROR, "Stop Database VM retry is disabled"
            )

        if error:
            return HttpResponseRedirect(
                reverse(
                    'admin:maintenance_databasestopdatabasevm_change',
                    args=(stop_database_vm_id,)
                )
            )
