...
class DatabaseFlipperFoxMigration(BaseModel):
    database = models.ForeignKey(Database, verbose_name=("Database"),
                                 null=False, unique=True,
                                 related_name="flipperfoxmigration")
    ...
    def get_steps(self,):
        return get_flipeerfox_migration_steps()
    ...
    class Meta:
        permissions = (
            ("view_databaseflipperfoxmigration",
             "Can view database flipperfox migration"),
        )


class DatabaseFlipperFoxMigrationDetail(BaseModel):
    WAITING = 0
    RUNNING = 1
    SUCCESS = 2
    ROLLBACK = 3
    ...
    database_flipperfox_migration = models.ForeignKey(DatabaseFlipperFoxMigration,
    ...
    class Meta:
        unique_together = (
            ('database_flipperfox_migration', 'step', 'scheduled_for',)
        )
        permissions = (
            ("view_databaseflipperfoxmigrationdetail",
             "Can view database flipperfox migration detail"),
        )

    def __unicode__(self):
        return " Detail for %s" % (self.database_flipperfox_migration)
...
simple_audit.register(DatabaseFlipperFoxMigration, DatabaseFlipperFoxMigrationDetail)
