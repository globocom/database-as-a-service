...
from ..service.databaseflipperfoxmigrationdetail import DatabaseFlipperFoxMigrationDetailService
from .. import models
import logging
...
class DatabaseFlipperFoxMigrationDetailAdmin(admin.DjangoServicesAdmin):
    actions = None
    service_class = DatabaseFlipperFoxMigrationDetailService
    search_fields = ("step", "status",)
    list_display = ("database_flipperfox_migration", "step_friendly", "scheduled_for",
                    "friendly_status", "friendly_direction", "started_at",
                    "finished_at", "created_by", "revoked_by",)
    fields = ("database_flipperfox_migration", "step_friendly", "scheduled_for",
    ...
    def get_readonly_fields(self, request, obj=None):
        detail = obj
        self.max_num = None
        custom_template = "admin/flipperfox_migration/databaseflipperfoxmigrationdetail/custom_change_form.html"
        if detail and detail.status == models.DatabaseFlipperFoxMigrationDetail.WAITING:
    ...
    def revoke_detail(request, id):
        ...
        detail = models.DatabaseFlipperFoxMigrationDetail.objects.get(id=id)
        ...
        return HttpResponseRedirect(reverse('admin:flipperfox_migration_databaseflipperfoxmigrationdetail_changelist'))
    ...
    def change_view(self, request, object_id, form_url='', extra_context={}):
        ...
        return super(DatabaseFlipperFoxMigrationDetailAdmin, self).change_view(request,
                                                                           object_id,
                                                                           form_url,
    ...
    def get_urls(self):
        ...
        urls = super(DatabaseFlipperFoxMigrationDetailAdmin, self).get_urls()
        my_urls = list(
    ...
    def friendly_status(self, detail):
        ...
        if detail.status == models.DatabaseFlipperFoxMigrationDetail.SUCCESS:
            return format_html(html_success)
        elif detail.status == models.DatabaseFlipperFoxMigrationDetail.ROLLBACK:
            return format_html(html_rejected.format("Rollback"))
        elif detail.status == models.DatabaseFlipperFoxMigrationDetail.WAITING:
            return format_html(html_waiting)
        elif detail.status == models.DatabaseFlipperFoxMigrationDetail.RUNNING:
            return format_html(html_running)
        elif detail.status == models.DatabaseFlipperFoxMigrationDetail.REVOKED:
            return format_html(html_revoked.format("Revoked"))
        elif detail.status == models.DatabaseFlipperFoxMigrationDetail.ERROR:
