...
SELECT now(), now(), physical_enginetype.id,
    ...
    '', 'string'
from physical_enginetype where name = 'mysql' or name = 'mysql_percona';
...
AND physical_parameter.engine_type_id IN (select id from physical_enginetype where name = "mysql" or name = "mysql_percona");
